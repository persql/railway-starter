...
// PerSQL connection — the three vars come from https://railway.persql.com/connect
// and are set in your Railway service's Variables tab (or a shared group).
...
// Run-once schema setup. IF NOT EXISTS keeps this idempotent across restarts.
...
const server = createServer(async (req, res) => {
  ...
  if (!configured) {
    ...
    res.end(
      page(
        "Not connected yet",
        "Set <code>PERSQL_TOKEN</code> and <code>PERSQL_DATABASE</code> (and optionally <code>PERSQL_API_URL</code>) on this Railway service. Get them at <a href=\"https://railway.persql.com/connect\">railway.persql.com/connect</a>.",
      ),
    );
    return;
  }
...
<p style="color:#a3a3a3;font-size:13px">PerSQL railway starter &middot; <a href="https://persql.com">persql.com</a></p>`;
}
